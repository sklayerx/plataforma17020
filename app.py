...
# --- FUNCIONES DE PDF REESTRUCTURADAS (SECCIÓN 3 FOTOS, SECCIÓN 4 CONCLUSIÓN) ---
def generar_pdf_informe(datos_cabecera, datos_equipo, datos_tecnicos, respuestas, id_reporte, dictamen, inspector, lista_fotos):
    ...
    # REORGANIZACIÓN: SECCIÓN 3 AHORA ES EL REGISTRO FOTOGRÁFICO
    if lista_fotos:
    ...
    # REORGANIZACIÓN: SECCIÓN 4 AHORA ES LA CONCLUSIÓN Y FIRMAS
    story.append(Spacer(1, 15))
...
with st.form("formulario_checklist_completo"):
    ...
    # PLATAFORMA: CAMPO DE FOTOS (Sección 5 en pantalla, se mapeará como Sección 3 en el PDF)
    st.markdown("---")
...
if st.session_state.historial_inspecciones:
    ...
    st.warning("No hay informes registrados en la sesión actual.")    st.title("ANDES — Ingeniería y Servicios")
    st.subheader("Sistema de Gestión de Inspecciones Oficiales (ISO/IEC 17020)")
    st.caption("Plataforma de Captura de Datos en Campo — Módulo: Grúas Móviles (GM)")
